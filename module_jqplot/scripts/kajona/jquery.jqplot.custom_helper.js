...
define("jqlot.custom_helper", [
   ...
   */
  jqplotHelper.jqPlotChart = function(
    strChartId,
    strTooltipId,
    strResizeableId,
    bitEnableChartResizing,
    arrChartData,
    objChartOptions,
    objPostPlotOptions,
    arrSeriesToDataPoints,
    bitIsResponsive
  ) {
    ...
    this.objChartOptions.axesDefaults.tickOptions.formatter =
      jqplotHelper.customJqPlotNumberFormatter;
    ...
    this.postPlot = function() {
      if (
        this.objPostPlotOptions.hasOwnProperty("intNrOfWrittenLabelsXAxis") &&
        this.objPostPlotOptions["intNrOfWrittenLabelsXAxis"] != null
      ) {
        this.objJqplotChart.postDrawHooks.add(jqplotHelper.setLabelsInvisible, [
          this.strChartId,
          this.objPostPlotOptions["intNrOfWrittenLabelsXAxis"],
          "xaxis"
        ]);
        jqplotHelper.setLabelsInvisible(
          this.strChartId,
          this.objPostPlotOptions["intNrOfWrittenLabelsXAxis"],
          "xaxis"
        );
      ...
      if (
        this.objPostPlotOptions.hasOwnProperty("intNrOfWrittenLabelsYAxis") &&
        this.objPostPlotOptions["intNrOfWrittenLabelsYAxis"] != null
      ) {
        this.objJqplotChart.postDrawHooks.add(jqplotHelper.setLabelsInvisible, [
          this.strChartId,
          this.objPostPlotOptions["intNrOfWrittenLabelsYAxis"],
          "yaxis"
        ]);
        jqplotHelper.setLabelsInvisible(
          this.strChartId,
    ...
    this.plot = function() {
      ...
      if (this.bitIsResponsive) {
        jqplotHelper.enableChartResponsive(
          this.strChartId,
          this.strResizeableId
        );
      }
    };
  ...
  jqplotHelper.bindDataClickEvents = function(strChartId) {
    $("#" + strChartId).bind("jqplotDataClick", function(
      ev,
      seriesIndex,
      pointIndex,
      data
      ...
      if (
        objChart.arrSeriesToDataPoints &&
        objChart.arrSeriesToDataPoints[seriesIndex]
      ) {
        if (objChart.arrSeriesToDataPoints[seriesIndex][pointIndex]) {
          var objDataPoint =
            objChart.arrSeriesToDataPoints[seriesIndex][pointIndex];

          if (
            objDataPoint.actionhandler &&
            objDataPoint.actionhandler != null
          ) {
            ...
            jqplotHelper.dataPointOnClickURLHandler(
              ev,
              seriesIndex,
              pointIndex,
              data,
  ...
  jqplotHelper.bindMouseEvents = function(strChartId, strTooltipId) {
    $("#" + strChartId).bind("jqplotMouseMove", function(
      ev,
      gridpos,
      datapos,
      neighbor,
    ...
    $("#" + strChartId).bind("jqplotMouseLeave", function(
      ev,
      gridpos,
      datapos,
      neighbor,
   ...
   * @param strAxis
   */
  jqplotHelper.setLabelsInvisible = function(
    strChartId,
    intNoOfWrittenLabels,
    strAxis
  ) {
  ...
  jqplotHelper.mouseLeave = function(
    ev,
    gridpos,
    datapos,
    neighbor,
  ...
  jqplotHelper.mouseMove = function(
    ev,
    gridpos,
    datapos,
    neighbor,
    ...
    if (neighbor == null) {
      ...
      if (series._type == "pie") {
        ...
        xValue = isNaN(neighbor.data[0])
          ? neighbor.data[0]
          : Math.round(neighbor.data[0] * 1000) / 1000;
        yValue = isNaN(neighbor.data[1])
          ? neighbor.data[1]
        ...
        xValue =
          arrXAxisTicks.length > 0 && series._primaryAxis == "_xaxis" //only if xaxis is primary axis use the tick labels
            ? arrXAxisTicks[neighbor.pointIndex]
            : Math.round(neighbor.data[0] * 1000) / 1000;

        //Either set label value for this datapoint or value (if no y-axis labels were set)
        yValue =
          arrYAxisTicks.length > 0 && series._primaryAxis == "_yaxis" //only if yaxis is primary axis use the tick labels
      ...
      //new data point
      if (
        previousNeighbor == null || //new data point --> create new point
        previousNeighbor.seriesIndex != neighbor.seriesIndex || //different series --> create new point
        (previousNeighbor.seriesIndex == neighbor.seriesIndex &&
          previousNeighbor.pointIndex != neighbor.pointIndex)
      ) {
        //same series but different point --> create new point
        jqplotHelper.showTooltip(
          ev.pageX,
          ev.pageY,
          objTooltip,
          tooltipId,
  ...
  jqplotHelper.hasDataPoint = function(ev, seriesIndex, pointIndex) {
    ...
    if (
      ...
      if (
        objDataPoint &&
        objDataPoint.actionhandlervalue &&
        objDataPoint.actionhandlervalue != null
      ) {
        return true;
      }
    }
  ...
  jqplotHelper.showTooltip = function(x, y, objTooltip, tooltipId, move) {
    ...
    if (!move) {
      ...
      if ($.isNumeric(valuePrimaryAxis)) {
        valuePrimaryAxis = tickOptionsrimaryAxis.formatter(
          tickOptionsrimaryAxis.formatString,
          valuePrimaryAxis
        );
      }
      if ($.isNumeric(valueSecondaryAxis)) {
        valueSecondaryAxis = tickOptionsSecondaryAxis.formatter(
          tickOptionsSecondaryAxis.formatString,
          valueSecondaryAxis
        );
      }
      ...
      var toolTipDiv = $(
        '<div id="jqplot_tooltip" class="jqplot-chart-tooltip">' +
      ...
      $("#jqplot_tooltip_content").html(
        "<span>" +
          objTooltip.seriesLabel +
          " : <b>" +
          valueSecondaryAxis +
          "</b></span>"
      );
      ...
      toolTipDiv
        .css("border-color", objTooltip.seriesColor)
        .css("top", top)
        .css("left", left)
        .show();
    } else {
  ...
  jqplotHelper.dataPointOnClickURLHandler = function(
    ev,
    seriesIndex,
    pointIndex,
    data,
  ...
  jqplotHelperdataRedirect = function(
    ev,
    seriesIndex,
    pointIndex,
    data,
