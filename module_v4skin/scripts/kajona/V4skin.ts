...
class MessagingOptions {
    ...
    public static pollMessages() {
        Messaging.getRecentMessages(function (objResponse: any) {
            ...
            $('#messagingShortlist').append("<li class='divider'></li><li><a href='#/messaging'><i class='fa fa-envelope'></i> " + V4skin.messaging.properties.show_all + "</a></li>");
...
class V4skin {
    ...
    public static initTagMenu(){
        Ajax.genericAjaxCall("tags", "getFavoriteTags", "", function(data: any, status: string, jqXHR: XMLHttpRequest) {
            if(status == 'success') {
                $('#tagsSubemenu').empty();
    ...
    public static removeObjectListItem(el: any){
        ...
        if (table.siblings('.initval-marker').length > 0) {
            ...
            marker = marker.replace($(el).closest('tr').find('input').val() + "", "");
     ...
     */
    public static setObjectListItems(strElementName: string, arrItems: Array<any>, arrAvailableIds: Array<string>, strDeleteButton: string){
     ...
     * @param arrItems
     */
    public static setCheckboxArrayObjectListItems(strElementName: string, arrItems: Array<any>){
     ...
     * @param strDeleteButton
     */
    public static updateCheckboxTreeSelection(objIframeEl: any, strElementName: string, strDeleteButton: string){
