...
class ListSortable {
     ...
     * @param bitMoveToTree
     */
    public static init(strListId : string, strTargetModule : string, bitMoveToTree : boolean) {

        var $objListNode = $("#"+strListId);

        var oldPos : number = null;
        var intCurPage = parseInt($objListNode.attr("data-kajona-pagenum"));
        var intElementsPerPage = parseInt($objListNode.attr("data-kajona-elementsperpage"));
        ...
        $('#'+strListId+'_prev').sortable({
            ...
            receive: function (event, ui) {
                $(ui.placeholder).hide();
                if (intCurPage > 1) {
                    Ajax.setAbsolutePosition(ui.item.find('tr').data('systemid'), (intElementsPerPage * (intCurPage - 1)), null, function (data : any, status : string, jqXHR : any) {
                        if (status == 'success') {
                            location.reload();
                        }
                        else {
        ...
        $objListNode.sortable({
            ...
            start: function (event, ui) {
                let pageNum = parseInt($("#"+strListId).attr("data-kajona-pagenum"));
                if (pageNum > 1) {
                    $('#'+strListId+'_prev').css("display", "block");
                }

                let elPerPage = parseInt($("#"+strListId).attr("data-kajona-elementsperpage"));
                if (($('#'+strListId+' tbody:has(tr[data-systemid!=""][data-systemid!="batchActionSwitch"])').length -1 ) >= elPerPage) {
            ...
            stop: function (event, ui) {
                if (oldPos != ui.item.index() && !ui.item.parent().is('div')) {
                    ...
                    var intCurPage = parseInt($("#"+strListId).attr("data-kajona-pagenum"));
                    var intElementsPerPage = parseInt($("#"+strListId).attr("data-kajona-elementsperpage"));
        ...
        $('#'+strListId +' > tbody:has(tr[data-systemid!=""][data-deleted=""][data-systemid!="batchActionSwitch"]) > tr').each(function (index) {
            ...
            var self = this;
            Lang.fetchSingleProperty("commons", "commons_sort_vertical", function(strText : string) {
            ...
            if (bitMoveToTree) {
                ...
                Lang.fetchSingleProperty("commons", "commons_sort_totree", function(strText : string) {
