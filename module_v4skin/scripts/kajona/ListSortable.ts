  ...
  public static init(
    ...
    $("#" + strListId + "_next").sortable({
      ...
      receive: function(event, ui) {
        $(ui.placeholder).hide();
        var intOnPage =
          $("#" + strListId + ' tbody:has(tr[data-systemid!=""])').length + 1;
        ...
        if (oldPos != ui.item.index() && !ui.item.parent().is("div")) {
          ...
          $("#" + strListId + " > tbody").each(function(index) {
            ...
            if (
              $(this)
                .find("tr")
                .data("systemid") == ui.item.find("tr").data("systemid")
            )
              return false;
          });
    ...
    $(
      ...
      var self = this;
      Lang.fetchSingleProperty("commons", "commons_sort_vertical", function(
        strText: string
      ) {
      ...
      if (bitMoveToTree) {
        ...
        Lang.fetchSingleProperty("commons", "commons_sort_totree", function(
          strText: string
        ) {
