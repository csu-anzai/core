...
abstract class Root
{
    ...
    public function copyObject($strNewPrevid = "", $bitChangeTitle = true, $bitCopyChilds = true)
    {
        ...
        if ($bitCopyChilds) {
            ...
            $arrChilds = $this->objDB->getPArray("SELECT system_id FROM "._dbprefix_."system where system_prev_id = ? WHERE system_deleted = 0 ORDER BY CASE WHEN system_sort < 0 THEN 9999999 ELSE system_sort END ASC, system_create_date DESC", array($strOldSysid));
            foreach ($arrChilds as $arrOneChild) {
                if (validateSystemid($arrOneChild["system_id"])) {
                    $objInstance = Objectfactory::getInstance()->getObject($arrOneChild["system_id"]);
                    if ($objInstance !== null) {
