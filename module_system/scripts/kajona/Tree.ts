...
class JSTree {
    ...
    public moveNode(data : any) {
        ...
        if (strNewParentId == strOldParentId) {
            ...
            /* Call server*/
            Ajax.genericAjaxCall("system", "setAbsolutePosition", strNodeId + "&listPos=" + (intNewPostiton + 1), function (data : any, status : string, jqXHR : XMLHttpRequest) {
        ...
        //different parent
            ...
            /* Call server*/
            Ajax.genericAjaxCall("system", "setPrevid", strNodeId + "&prevId=" + strNewParentId, function (data : any, status : string, jqXHR : XMLHttpRequest) {
     ...
     * @returns {boolean}
     */
    public checkMoveNode(node : any, node_parent : any, node_position : number, more : any) {
    ...
    public isValidParentNodeForChild(node : any, node_parent : any) {
        var nodeWithDataAttribute = this.getNodeWithDataAttribute(node, 'check_parent_id_active', true);
     ...
     */
    public getNodeWithDataAttribute(node : any, strAttribute : string, bitCheckParentNodesOnly? : boolean) {
    ...
    public initTree() {
        ...
        var jsTreeObj : JSTreeStaticDefaults = {
            'core': {
                 ...
                 * @returns {boolean}
                 */
                'check_callback': function (operation : string, node : any, node_parent : any, node_position : number, more : any) {
                    ...
                    if (operation === 'move_node') {
                        ...
                        bitReturn = true;
                        if (more.dnd) {
                            bitReturn = treeContext.checkMoveNode(node, node_parent, node_position, more);
                        }
                    }
                ...
                'data': {
                    ...
                    'data': function (node : any, cb : any) {//params to be added to the given ulr on ajax call
                        var data : any = {};
                        if (node.id === "#") {
