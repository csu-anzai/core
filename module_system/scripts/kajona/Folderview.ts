    ...
    if (tbody.length > 0) {
      ...
      for (var i = 0; i < arrItems.length; i++) {
        ...
        if (arrItems[i].strEditLink) {
          html +=
            '    <td class="icon-cell">' + arrItems[i].strEditLink + "</td>";
        }
        html += "</tr>";

        if (
          tbody.find("tr[data-kajona-systemid=" + arrItems[i].strSystemId + "]")
            .length > 0
        ) {
          tbody
            .find("tr[data-kajona-systemid=" + arrItems[i].strSystemId + "]")
            .replaceWith(html);
        } else {
          tbody.append(html);
        }
