...
class SystemTask {
    ...
    public static executeTask(strTaskname : string, strAdditionalParam : string, bitNoContentReset : boolean) {
        if (bitNoContentReset === null || bitNoContentReset === undefined) {
            ...
            $("#"+jsDialog_0.getContainerId()).find("div.modal-dialog").removeClass("modal-lg");
            $('#systemtaskCancelButton').click(SystemTask.cancelExecution);
            jsDialog_0.init();
        }

        Ajax.genericAjaxCall("system", "executeSystemTask", "&task="+strTaskname+strAdditionalParam, function(data : any, status : string, jqXHR : XMLHttpRequest) {
            if(status == 'success') {
                ...
                if(strResponseText.indexOf("<error>") != -1) {
                    ...
                    if(strReload == "") {
                        ...
                        $('#systemtaskCancelButton').attr("value", KAJONA_SYSTEMTASK_CLOSE);
                    }
                    else {
