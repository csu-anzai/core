...
class Imageeditor {
    ...
    public static saveImageRotating(intAngle: number, strFile: string, objCallback: Function) {
    ...
    public static saveCroppingToBackend() {
        ...
        if (this.fm_image_isScaled) {
            ...
            var intScaledWidth = parseInt($('#fm_mediamanagerPic').attr("width"));
            var intScaledHeight = parseInt($('#fm_mediamanagerPic').attr("height"));
            var intOriginalWidth = parseInt($('#fm_int_realwidth').attr("value"));
            var intOriginalHeigth = parseInt($('#fm_int_realheight').attr("value"));
        ...
        var iE = this;
        var callback = function (data: any, status: string, jqXHR: XMLHttpRequest) {
            if (status == 'success') {
